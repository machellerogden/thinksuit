...
// a console restart. It wraps trainWakeword() (which registers the exported model
// as a new version on success) and records everything to the wakeword's run-log,
// so progress and the final result are durable, inspectable artifacts. On a first
// train (no promoted version yet) it auto-promotes so the wakeword is immediately
// usable; a re-train registers a version but leaves promotion to the user.

import process from 'node:process';
import * as store from '../src/wakewords/store.js';
import { trainWakeword } from '../src/wakewords/trainer.js';
